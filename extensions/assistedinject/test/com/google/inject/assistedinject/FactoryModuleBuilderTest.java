...
public class FactoryModuleBuilderTest extends TestCase {
  ...
  public void testImplicitForwardingAssistedBindingFailsWithInterface() {
	    try {
	      ...
	      assertContains(
	          ce.getMessage(),
	          new StringBuilder().append("1) ").append(Car.class.getName()).append(" is an interface, not a concrete class.").toString(),
	          "Unable to create AssistedInject factory.",
	          "while locating " + Car.class.getName(),
	          new StringBuilder().append("at ").append(ColoredCarFactory.class.getName()).append(".create(").toString());
	...
	public void testImplicitForwardingAssistedBindingFailsWithAbstractClass() {
	    try {
	      ...
	      assertContains(
	          ce.getMessage(),
	          new StringBuilder().append("1) ").append(AbstractCar.class.getName()).append(" is abstract, not a concrete class.").toString(),
	          "Unable to create AssistedInject factory.",
	          "while locating " + AbstractCar.class.getName(),
	          new StringBuilder().append("at ").append(ColoredAbstractCarFactory.class.getName()).append(".create(").toString());
	      assertEquals(1, ce.getErrorMessages().size());
	    }
	  }
	...
	public void testExplicitForwardingAssistedBindingFailsWithInterface() {
	    try {
	      ...
	      assertContains(
	          ce.getMessage(),
	          new StringBuilder().append("1) ").append(Volkswagen.class.getName()).append(" is an interface, not a concrete class.").toString(),
	          "Unable to create AssistedInject factory.",
	...
	public void testSimilarBindingsWithConflictingImplementations() {
	    try {
	      ...
	      assertContains(
	          ce.getMessage(),
	          new StringBuilder().append("A binding to ").append(ColoredCarFactory.class.getName()).append(" was already configured").toString());
	      assertEquals(1, ce.getErrorMessages().size());
	    }
	  }
	...
	public void testFactoryPublicAndReturnTypeNotPublic() {
	    try {
	      ...
	      assertEquals(
	          new StringBuilder().append(NotHidden.class.getName()).append(" is public, but has a method that returns a non-public type: ").append(Hidden.class.getName()).append(". Due to limitations with java.lang.reflect.Proxy, this is not allowed. ").append("Please either make the factory non-public or the return type public.").toString(),
	...
	public void testSingletonScopeOnAssistedClassIsIgnored() {
	    try {
	      ...
	      assertEquals(1, ce.getErrorMessages().size());
	      assertEquals(
	          new StringBuilder().append("Found scope annotation [").append(Singleton.class.getName()).append("]").append(" on implementation class [").append(AssistedSingleton.class.getName()).append("]")
					.append(" of AssistedInject factory [").append(SingletonFactory.class.getName()).append("].").append("\nThis is not allowed, please remove the scope annotation.").toString(),
	          Iterables.getOnlyElement(ce.getErrorMessages()).getMessage());
	    }
	  }
