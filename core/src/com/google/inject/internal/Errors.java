  ...
  <T> Errors missingImplementationWithHint(Key<T> key, Injector injector) {
    ...
    if (!sameTypes.isEmpty()) {
      ...
      if ((possibleMatches.size() > 0) && (possibleMatches.size() <= MAX_RELATED_TYPES_REPORTED)) {
        sb.append(format("%n  Did you mean?"));
        possibleMatches.forEach(possibleMatch -> sb.append(format("%n    %s", possibleMatch)));
      }
    }
...
public Errors conversionTypeError(
      ...
      Object converted) {
    return addMessage(
        new StringBuilder().append("Type mismatch converting '%s' (bound at %s) to %s%n").append(" using %s.%n").append(" Converter returned %s.").toString(),
...
public Errors ambiguousTypeConversion(
    ...
    return addMessage(
        new StringBuilder().append("Multiple converters can convert '%s' (bound at %s) to %s:%n").append(" %s and%n").append(" %s.%n").append(" Please adjust your type converter configuration to avoid overlapping matches.").toString(),
...
public Errors childBindingAlreadySet(Key<?> key, Set<Object> sources) {
    ...
    Errors errors =
        addMessage(
            new StringBuilder().append("Unable to create binding for %s.").append(" It was already configured on one or more child injectors or private modules").append("%s%n").append("  If it was in a PrivateModule, did you forget to expose the binding?").toString(),
            key, allSources.out());
    return errors;
  }
...
public Errors merge(Collection<Message> messages) {
    ...
    messages.forEach(message -> addMessage(Messages.mergeSources(sources, message)));
    return this;
  }
